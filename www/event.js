...
function FindGood(inpGoodCode) {
    ...
    $.ajax({
        ...
        complete: function() {
            ...
            var touchtime_ClearCount = 0;
            $("#modal_Count").on("click", function() {

                if (touchtime_ClearCount == 0) {

                    touchtime_ClearCount = new Date().getTime();

                } else {
                    if (((new Date().getTime()) - touchtime_ClearCount) < 300) {

                        //alert("Click Input Goods");
                        $('#modal_Count').val("");
                        touchtime_ClearCount = 0;
